...
export class BlockchainService {
  private simulatedBalances: Map<string, string> = new Map();
  private simulatedNFTs: Map<string, any[]> = new Map();
  ...
  async transferTokens(from: string, to: string, amount: string): Promise<boolean> {
    ...
    await UserModel.findOneAndUpdate(
      { walletAddress: from },
      { $inc: { currentCademBalance: -transferAmount } }
    );
  ...
  async mintNFT(to: string, metadata: any): Promise<boolean> {
