...
router.get('/:type/position/:walletAddress', async (req, res) => {
    try {
        ...
        const user = await User_1.UserModel.findOne({ walletAddress });
        if (!user) {
...
router.get('/stats/overview', async (req, res) => {
    try {
        const totalUsers = await User_1.UserModel.countDocuments();
        const totalTokensEarned = await User_1.UserModel.aggregate([
            { $group: { _id: null, total: { $sum: '$totalCademEarned' } } }
        ]);
        const averageLevel = await User_1.UserModel.aggregate([
            { $group: { _id: null, avg: { $avg: '$level' } } }
        ]);
        const topPlayer = await User_1.UserModel.findOne()
            .sort({ totalCademEarned: -1 })
            .select('walletAddress username totalCademEarned level');
        const recentPlayers = await User_1.UserModel.find()
            .sort({ createdAt: -1 })
            .limit(5)
